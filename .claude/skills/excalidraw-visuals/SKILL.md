...
Healthcare-specific color assignments:
- **Payor elements:** Soft blue (#a5d8ff)
- **Provider elements:** Teal (#81D4C2)
- **Patient elements:** Warm yellow (#ffec99)
- **Data elements:** Light purple (#d0bfff)
- **Alerts / problems:** Coral (#ffa8a8)
- **Flows:** Teal (input) -> Blue (process) -> Yellow (output)
...
You can pass additional reference images via extra `--input` arguments:
```bash
...
| Concept | Icon |
|---------|------|
| Payor / Insurance | Shield with dollar sign |
| Provider / Hospital | Building with cross |
| Provider / Doctor | Stick figure with stethoscope |
| Patient | Stick figure (plain) |
| Family / Dependents | Two stick figures (tall + short) |
| Data / Records | Document with folded corner |
| HIPAA / Security | Lock icon |
| Coding (ICD/CPT) | Document with # symbol |
| Claims | Document with arrow |
| Money / Payment | Dollar sign in circle |
